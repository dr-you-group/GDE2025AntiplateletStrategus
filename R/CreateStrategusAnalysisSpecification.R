...
studyStartDate <- '20090710' #YYYYMMDD
...
oList <- cohortDefinitionSet %>%
  filter(.data$cohortId %in% c(1792367, # [GDE2025_Antiplatelet] Primary outcome - MACE
...
cmTcList <- data.frame(
  targetCohortId =  1792797, # [GDE2025_Antiplatelet] Ticagrelor cohort - Revised
  targetCohortName = "Ticagrelor cohort",
  comparatorCohortId =  1793457, # [GDE2025_Antiplatelet] Prasugrel cohort - Revised
  comparatorCohortName = "Prasugrel cohort"
)

# For the CohortMethod LSPS we'll need to exclude the drugs of interest in this
# study
excludedCovariateConcepts <- read.csv(file = "inst/Excludedcovariate.csv") 
...
for (i in seq_len(nrow(timeAtRisks))) {
  ...
  cmAnalysisList[[i + length(timeAtRisks) - 1]] <- CohortMethod::createCmAnalysis(
    analysisId = i + length(timeAtRisks) - 1,
    description = sprintf(
      "Cohort method, %s",
      timeAtRisks$label[i]
    ),
...
ParallelLogger::saveSettingsToJson(
  ...
  file.path("inst", "GDE2025AniplateletAnalysisSpecification.json")
)
